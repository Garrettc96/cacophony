...
public class ModelMapper {
    ...
    public User requestToUser(CreateUserRequest createUserRequest) {
        return User.builder().email(createUserRequest.getEmail()).password(createUserRequest.getPassword())
                .username(createUserRequest.getUsername()).createdAt(OffsetDateTime.now()).build();
    }
    ...
    public Chat requestToChat(CreateChatRequest createChatRequest) {
        return Chat.builder().name(createChatRequest.getName()).description(createChatRequest.getDescription())
                .conversation(Conversation.builder().type(ConversationType.CHAT)
                        .members(createChatRequest.getMembers().stream().map(User::fromId).toList()).build())
                .build();
    }

    public CreateChatResponse chatToCreateResponse(Chat chat) {
        return CreateChatResponse.builder().id(chat.getId()).name(chat.getName()).description(chat.getDescription())
                .members(chat.getConversation().getMembers().stream().map(User::getId).toList()).build();
    }
    ...
    public Channel requestToChannel(CreateChannelRequest request) {
        return Channel.builder().description(request.getDescription()).name(request.getName())
                .conversation(Conversation.builder().type(ConversationType.CHANNEL)
                        .members(request.getMembers().stream().map(User::fromId).toList()).build())
                .build();
    }

    public CreateChannelResponse channelToCreateResponse(Channel channel) {
        return CreateChannelResponse.builder().id(channel.getId()).name(channel.getName())
                .description(channel.getDescription())
                .members(channel.getConversation().getMembers().stream().map(User::getId).toList()).build();
    }
