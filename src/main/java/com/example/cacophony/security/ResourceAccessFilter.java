...
record AuthTypeAndId(String id, ModelType resourceType) {
}
...
public class ResourceAccessFilter extends OncePerRequestFilter {
    ...
    public ResourceAccessFilter(MessageService messageService, ConversationService conversationService,
            UserService userService, ChatService chatService, ChannelService channelService) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        AuthTypeAndId authTypeAndId = switch (resourceAccessType) {
            case ResourceAccessType.ResourceAccessPathId t -> parseAuthTypeAndIdFromRequest(fullRequestPathWithQuery);
    ...
    private boolean canUserAccessAuthTypeAndId(UserInfoDetails userDetails, AuthTypeAndId authTypeAndId) {
        ...
        return switch (authTypeAndId.resourceType()) {
            ...
            case CONVERSATION -> this.conversationService.isUserInConversation(UUID.fromString(authTypeAndId.id()),
                    userDetails.userId);
        };
    }
    ...
    private AuthTypeAndId parseAuthTypeAndIdFromBody(HttpServletRequest request,
            ResourceAccessType.ResourceAccessBodyId resourceAccessBodyType) {
            ...
            case ResourceAccessType.ResourceAccessBodyId t -> {
                try {
                    ...
                    for (Class subClass : ResourceAuthorizationBody.class.getPermittedSubclasses()) {
                        try {
                            ResourceAuthorizationBody body = (ResourceAuthorizationBody) MAPPER.readValue(lines,
                                    subClass);
